"""
ECG Location Prediction - CNN-RNN + Attention with 2-Stage GridSearch
...
print("=" * 80)
print("CNN-RNN + Attention - 2-Stage GridSearch")
print("=" * 80)
...
def build_cnn_rnn_attention(input_shape, num_classes, 
    ...
    x = layers.SimpleRNN(rnn_units//2, return_sequences=True)(x)
    x = layers.Dropout(dropout_rate)(x)
    ...
    model = models.Model(inputs=inputs, outputs=outputs, name='CNN_RNN_Attention_GS')
...
def quick_screening(X, y, param_grid):
    ...
    for idx, param_values in enumerate(param_combinations):
        ...
        model = build_cnn_rnn_attention(
            ...
            rnn_units=params.get('rnn_units', 128)
        )
...
def full_validation(X, y, best_params, n_splits=5):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        model = build_cnn_rnn_attention(
            ...
            rnn_units=best_params.get('rnn_units', 128)
        )
...
def main():
    ...
    print("\n" + "=" * 80)
    print("2-STAGE GRIDSEARCH: CNN-RNN + Attention")
    print("=" * 80)
    ...
    df_screening.to_csv('GridSearch_Stage1_Screening_RNN.csv', index=False)
    ...
    df_folds.to_csv('GridSearch_Stage2_FoldResults_RNN.csv', index=False)
    
    # Save summary
    with open('GridSearch_Final_Summary_RNN.txt', 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("GridSearch Final Summary - CNN-RNN + Attention\n")
        f.write("=" * 80 + "\n\n")
    ...
    plt.bar(x_pos, accuracies, alpha=0.7, color='darkorange')
    plt.xlabel('Configuration Index')
    plt.ylabel('Accuracy')
    plt.title('Stage 1: All Configurations - RNN', fontweight='bold')
    plt.grid(True, alpha=0.3, axis='y')
    
    plt.subplot(2, 2, 2)
    fold_accs = [r['accuracy'] for r in final_results['fold_results']]
    folds = [r['fold'] for r in final_results['fold_results']]
    plt.bar(folds, fold_accs, alpha=0.7, color='crimson')
    plt.axhline(y=final_results['mean_accuracy'], color='r', linestyle='--')
    plt.xlabel('Fold')
    plt.ylabel('Accuracy')
    plt.title('Stage 2: Best Config 5-Fold CV - RNN', fontweight='bold')
    plt.grid(True, alpha=0.3, axis='y')
    ...
    plt.title('Learning Rate Impact - RNN', fontweight='bold')
    ...
    plt.bar(range(len(dropout_groups)), dropout_groups.values, alpha=0.7, color='forestgreen')
    plt.xlabel('Dropout Rate')
    plt.ylabel('Mean Accuracy')
    plt.title('Dropout Impact - RNN', fontweight='bold')
    plt.xticks(range(len(dropout_groups)), [f'{d:.1f}' for d in dropout_groups.index])
    plt.grid(True, alpha=0.3, axis='y')
    
    plt.tight_layout()
    plt.savefig('GridSearch_Complete_Visualization_RNN.png', dpi=300)
