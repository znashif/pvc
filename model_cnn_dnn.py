"""
ECG Location Prediction - CNN-DNN Model  
...
print("=" * 80)
print("CNN-DNN Model for ECG Location Prediction")
print("=" * 80)
...
def build_cnn_dnn(input_shape, num_classes):
    ...
    model = models.Model(inputs=inputs, outputs=outputs, name='CNN_DNN')
...
def plot_dnn_report(history, model, X_test, y_test, fold_no, class_names):
    """Generate comprehensive report plots for CNN-DNN"""
    suffix = "_CNNDNN_Default_GAN"
    ...
    plt.title('Confusion Matrix - CNN-DNN', fontweight='bold')
    ...
    plt.title('ROC Curve - CNN-DNN', fontsize=14, fontweight='bold')
    ...
    # 4. Feature Visualization (DNN doesn't have attention, skip)
    print("Skipping Attention Map (not applicable for DNN)...")
    ...
    try:
        ...
        plt.title('t-SNE Visualization - CNN-DNN', fontsize=14, fontweight='bold')
...
def main():
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):
        ...
        checkpoint = ModelCheckpoint(
            f'model_cnn_dnn_fold{fold+1}.h5',
        ...
        # Load best model
        model.load_weights(f'model_cnn_dnn_fold{fold+1}.h5')
        ...
        if accuracy > best_acc:
            ...
            model.save('best_model_cnn_dnn_gan.h5')
    ...
    # ==========================================
    print("\n" + "="*80)
    print("📊 Generating Report for CNN-DNN...")
    print("="*80)
    
    plot_dnn_report(history_best, best_model, X_test_best, y_test_best, 0, le.classes_)
    ...
    plt.bar(folds, accuracies, alpha=0.7, color='forestgreen', edgecolor='black')
    plt.axhline(y=np.mean(accuracies), color='r', linestyle='--', label=f'Mean: {np.mean(accuracies):.4f}')
    plt.xlabel('Fold', fontsize=12)
    plt.ylabel('Accuracy', fontsize=12)
    plt.title('Accuracy Comparison Across Folds - CNN-DNN', fontsize=14, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3, axis='y')
    plt.tight_layout()
    plt.savefig('Grafik_Perbandingan_Fold_CNN_DNN.png', dpi=300)
    plt.close()
    print("✓ Grafik Perbandingan Fold")
    
    # Plot 2: Box Plot - Consistency
    plt.figure(figsize=(10, 6))
    plt.boxplot([accuracies], labels=['CNN-DNN'], patch_artist=True,
                boxprops=dict(facecolor='lightgreen', alpha=0.7),
                medianprops=dict(color='red', linewidth=2))
    plt.ylabel('Accuracy', fontsize=12)
    plt.title('Model Consistency - CNN-DNN', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3, axis='y')
    plt.tight_layout()
    plt.savefig('Grafik_Konsistensi_CNN_DNN.png', dpi=300)
    plt.close()
    ...
    plt.ylabel('Accuracy', fontsize=12)
    plt.title('Training Time vs Accuracy - CNN-DNN', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('Grafik_Time_vs_Accuracy_CNN_DNN.png', dpi=300)
    plt.close()
    ...
    t1.to_csv("Tabel_1_Evaluasi_CNN_DNN_GAN.csv", index=False)
    ...
    t2.to_csv("Tabel_2_Detail_Kelas_CNN_DNN_GAN.csv", index=False)
    print("✓ Tabel 2: Detail Per Kelas")
    
    # Table 3: Efficiency
    params = best_model.count_params()
    size = os.path.getsize("best_model_cnn_dnn_gan.h5") / (1024*1024)
    t3 = pd.DataFrame({
    ...
    t3.to_csv("Tabel_3_Efisiensi_CNN_DNN_GAN.csv", index=False)
    ...
    t4.to_csv("Tabel_4_Konsistensi_CNN_DNN_GAN.csv", index=False)
    print("✓ Tabel 4: Konsistensi Model")
    
    # Save fold results
    df_res.to_csv("Hasil_CrossValidation_CNN_DNN_GAN.csv", index=False)
    print("✓ Hasil Cross-Validation")
    
    print("\n" + "="*80)
    print("✅ CNN-DNN (GAN) SELESAI")
    print("="*80)
    print(f"📁 Semua output tersimpan dengan suffix '_CNN_DNN_GAN'")
